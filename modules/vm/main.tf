...
resource "azurerm_resource_group" "IN_RG" {
  name     = "${var.resource_group}-Config-${var.environment}"
...
# Crear asociación entre subnet y grupos de seguridad
...
resource "azurerm_linux_virtual_machine" "IN-VM" {
  name                  = "${var.server_name}-Config-${var.environment}"
  resource_group_name   = azurerm_resource_group.IN_RG.name
  location              = var.location
  size                  = "Standard_B2s"
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo su -c 'echo \"MAPBOX_ACCESS_TOKEN=${var.mapbox_access_token}\" >> /home/adminuser/.env'",
